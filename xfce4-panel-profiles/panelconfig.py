...
import psutil
from gi.repository import Gdk, GdkX11, Gio, GLib
...
def get_monitor_config():
    ...
    for key in sorted(
        ret_monitors, key=lambda key: ret_monitors[key].get("order_score")
    ):
...
class PanelConfig(object):
    ...
    def from_xfconf(cls, xfconf):
        ...
        result = xfconf.call_sync(
            "GetAllProperties", GLib.Variant("(ss)", ("xfce4-panel", "")), 0, -1, None
        )
    ...
    def remap_screens(
        self, remove_extra_panels=True, remap_extra_panels=True, spread_panels=False
    ):
        ...
        unused_screens = [
            screen
            for screen in monitor_primary_order
            if screen not in used_screens.values()
        ]
        deleted_panels = []
        ...
        new_panel_keys = ", ".join(
            [
                f"<{index}>"
                for index in panel_config_orig.keys()
    ...
    def check_desktop(self, path):
        ...
        try:
            if keyfile.load_from_data(
                bytes.decode(), len(bytes), GLib.KeyFileFlags.NONE
            ):
    ...
    def to_xfconf(self, xfconf):
        ...
        dbus_proxy = Gio.DBusProxy.new_sync(
            conn, 0, None, destination, path, interface, None
        )
        ...
        if dbus_proxy is not None:
            ...
            try:
                xfconf.call_sync(
                    "ResetProperty",
                    GLib.Variant("(ssb)", ("xfce4-panel", "/", True)),
                    0,
                    -1,
                    None,
                )
            ...
            for pp, pv in sorted(self.properties.items()):
                xfconf.call_sync(
                    "SetProperty",
                    GLib.Variant("(ssv)", ("xfce4-panel", pp, pv)),
                    0,
                    -1,
                    None,
                )
            ...
            try:
                dbus_proxy.call_sync(
                    "Terminate", GLib.Variant("(b)", ("xfce4-panel",)), 0, -1, None
                )
    ...
    def rc_to_xfconf(self, xfconf, dict_config=None):
        ...
        if dbus_proxy is not None:
            ...
            try:
                dbus_proxy.call_sync(
                    "Terminate",
                    GLib.Variant("(b)", ("xfce4-panel-profiles",)),
                    0,
                    -1,
                    None,
                )
    ...
    def xfconf_to_rc(cls, xfconf):
        ...
        result = xfconf.call_sync(
            "GetAllProperties",
            GLib.Variant("(ss)", ("xfce4-panel-profiles", "")),
            0,
            -1,
            None,
        )
